...
namespace WordConnect
{
    public class GameController : SingletonComponent<GameController>, ISaveable
    {
        ...
        [SerializeField] private Image LetterWheelContainerImage;
        [SerializeField] private Image glowImage;
        [SerializeField] private GameObject rewardButton;
        [Space]
        [SerializeField][HideInInspector] private List<PackInfo> packInfos = null;

        public RectTransform GetWheelPos => letterWheel.GetComponent<RectTransform>();
        public RectTransform GetSelectedPos => selectedLetters.GetComponent<RectTransform>();
        ...
        private void Update()
        {
            ...
            if (isStartLevel)
            {
                ...
                timeSlider.fillAmount = goldTime / 60;
                gameTime += Time.deltaTime;

            }

            if (!isShowRewardButton && gameTime>1800)
            {
                isShowRewardButton = true;
                
              rewardButton.SetActive(AdsManager.instance.isReadyReward);
            }



            isMustRewardGoldTime = goldTime < 60 && isActiveGoldTimer;
        ...
        private bool isMustRewardGoldTime = false;
        public bool isActiveGoldTimer = false;
        private float gameTime = 0;
        private bool isShowRewardButton;
        ...
        public void StartLevel(PackInfo packInfo, CategoryInfo categoryInfo, LevelData levelData)
        {
            ...
            var packInfosIndex = packInfos.IndexOf(packInfo);

            camera.backgroundColor = backgroundColors[packInfosIndex];
            LetterWheelContainerImage.color = circleColors[packInfosIndex];
            glowImage.color = circleColors[packInfosIndex];

            effectList.ForEach(m => m.SetActive(false));
            effectList[packInfosIndex].SetActive(true);
            ...
            if (NumLevelsTillAdShows >= numLevelsTillAd)
            {
                // Try and show an ad
                /*				#if BBG_MT_ADS
        ...
        private void StartLevel(ActiveLevel level)
        {
            ...
            isShowRewardButton = false;

            isActiveGoldTimer = (Random.Range(0, 100) < 25);

            timerPanel.SetActive(isActiveGoldTimer);
        ...
        private void OnWordSelected(string word)
        {
            ...
            if (levelWordData != null)
            {
                ...
                letterWheel.Scramble(false, false);

                StartCoroutine(PopupWin.instance.Show());
        ...
        private void FoundExtraWord(ActiveLevel level, string word)
        {
            ...
            StartCoroutine(PopupWin.instance.Show(true));
        }
        ...
        private void CompleteLevel(ActiveLevel level)
        {
            ...
            if (!wasLevelCompleted)
            {
                ...
                if (isMustRewardGoldTime)
                {
                    ...
                    // Give the coins right away but don't update the text. This makes it so it the app exits the player has been given the coins
                    // but we don't want to update teh text until the animation happens on the complete popup
                    GiveCoins(categoryCoinsAwarded, false);
            ...
            object[] popupInData =
            {
                ...
                isLastLevel,isMustRewardGoldTime};
        ...
        private bool GetNextLevel(ActiveLevel level, out PackInfo packInfo, out CategoryInfo categoryInfo, out int levelIndex)
        {
            ...
            if (Random.Range(1, 10) > 5 && packIndex > 0)
            {
        ...
        internal void SetMainMenuBackgeound()
        {
            camera.backgroundColor = backgroundColors.FirstOrDefault();
        }
