...
namespace WordConnect
{
    public class LetterWheel : UIMonoBehaviour, IPointerDownHandler, IDragHandler, IPointerUpHandler
    {
        ...
        public void Scramble(bool animate, bool isRandom = true)
        {
            ...
            List<Vector2> letterPositions = PickRandomPositionsForLetters(!isRandom);

            hand.gameObject.SetActive(false);

            CheckTutorial(letterPositions);
        ...
        private void CheckTutorial(List<Vector2> letterPositions)
        {
            switch (GameController.Instance.LastCompletedLevelNumber)
            {
                case 0:
                    if (GameController.Instance.CurrentActiveLevel.levelSaveData.foundWords.Count<=0)
                    {
                        TutoriatLevel_1(letterPositions);
                    }
                    else
                    {
                        TutoriatLevel_2(letterPositions);
                    }
                    break;
                case 1:
                    if (GameController.Instance.CurrentActiveLevel.levelSaveData.foundWords.Count <= 0)
                    {
                        TutoriatLevel_3(letterPositions);
                    }
                    else
                    {
                        TutoriatLevel_4(letterPositions);
                    }
                    break;
            }
        }

        private async void TutoriatLevel_1(List<Vector2> letterPositions)
        {


           
            hand.SetSiblingIndex(transform.childCount);

            await Task.Delay(2000);

            hand.localPosition = letterPositions[0];
            hand.gameObject.SetActive(true);
            hand.DOLocalMove(letterPositions[1], 0.8f).OnComplete(() =>
            {
                hand.DOLocalMove(letterPositions[2], 0.8f).OnComplete(() =>
                {
                    hand.gameObject.SetActive(false);
                });
            });
        ...
        private async void TutoriatLevel_2(List<Vector2> letterPositions)
        {
        ...
        private async void TutoriatLevel_3(List<Vector2> letterPositions)
        {
            ...
            hand.localPosition = letterPositions[2];
            hand.gameObject.SetActive(true);
            hand.DOLocalMove(letterPositions[1], 0.8f).OnComplete(() =>
            {
                hand.DOLocalMove(letterPositions[0], 0.8f).OnComplete(() =>
                {
                    hand.gameObject.SetActive(false);
                });
            });
        }
        private async void TutoriatLevel_4(List<Vector2> letterPositions)
        {
        ...
        private List<Vector2> PickRandomPositionsForLetters(bool isNot = false)
        {

            List<Vector2> letterPositions = GetLetterPositions(letterUIs.Count);
            if (isNot) { return letterPositions; }
